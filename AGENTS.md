...
- Product **vision** (ATS + CRM, Tekforce, competitor ideas) lives in `docs/wiki/`. A clickable HTML prototype is in `src/deskflow/proto/` (`GET /`). Identity lives in SQLite (`src/deskflow/db/`). Do not add OAuth, Postgres, or a second language unless a human asks. The evaluate API stays the scoring MVP.
